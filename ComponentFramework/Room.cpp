...
Room::Room(Component* parent, const char* posx_, const char* negx_, const char* posy_, const char* negy_, const char* posz_, const char* negz_, float scale_):
	Actor(parent), textureID(0) {
	roomSize = scale_;
	posx = posx_;
	posy = posy_;
	posz = posz_;
...
bool Room::OnCreate()
{
	...
	AddComponent<TransformComponent>(nullptr, Vec3(0.0f, 0.0f, 0.0f), Quaternion(), Vec3(roomSize, roomSize, roomSize));
...
void Room::Render()
{
	...
	// Reset face culling to normal
	glCullFace(GL_BACK);
	glDisable(GL_CULL_FACE);
}

bool Room::LoadImages()
{
